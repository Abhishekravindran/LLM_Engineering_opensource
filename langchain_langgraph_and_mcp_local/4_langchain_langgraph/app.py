...
Local Models Edition — by Abhishek. Ported directly from abhishek/agents/4_langchain_langgraph/app.py.
The only change: `setup()` builds a local model (Ollama or Hugging Face,
auto-detected) and passes it into `Sidekick(model)`, instead of the
original's implicit `"openai:gpt-5.4-mini"`.
...
HEADER = """
<div id="header">
    <div class="context-label">Your personal co-worker — Local Models Edition, by Abhishek</div>
    <h1>Sidekick</h1>
...
def build_local_model():
    """Colab -> Hugging Face transformers pipeline. Local PC -> Ollama.
    Swap USE_OLLAMA below to force a backend regardless of environment."""
    IN_COLAB = importlib.util.find_spec("google.colab") is not None
    USE_OLLAMA = not IN_COLAB

    if USE_OLLAMA:
        from langchain_ollama import ChatOllama
        return ChatOllama(model="llama3.2:3b", temperature=0.3)
    else:
...
async def setup():
    sidekick = Sidekick(build_local_model())
...
async def reset(sidekick):
    if sidekick:
        sidekick.cleanup()
    new_sidekick = Sidekick(build_local_model())
